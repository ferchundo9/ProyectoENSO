...
public class GestorPacientes implements DatosPacientes {
	...
	public void darDeBaja(String dni_paciente) {
		...
		for (Paciente p : pacientes)
			if (p.getDNI().equals(dni_paciente))
				contenido = true;
		if (!contenido)
			return;
		this.pacientes = this.leerpacientes();
	...
	public ArrayList<Paciente> leerpacientes() {
		...
		try {
			...
			while (leerUsuarios.readRecord()) {
				...
				usuarios.add(new Paciente(dni, nombre, apellidos, nacimiento, descripcion, enfermedad, tratamiento,
						Boolean.parseBoolean(feq), Boolean.parseBoolean(temp), Boolean.parseBoolean(ten))); // Añade la
																											// informacion
																											// a la
																											// lista
			}
